...
fn generate(name: &'static str) {
    ...
    for (path_postfix, attribute_postfix) in [
        ("", None),
        ("-no-optimize", Some(quote!(#[pest_optimizer = false]))),
    ] {
        ...
        let path_expected = if cfg!(feature = "grammar-extras") {
            format!("tests/{name}{path_postfix}-expected-extras.rs")
        } else {
            format!("tests/{name}{path_postfix}-expected.rs")
        };
        let path_generated = format!("tests/{name}{path_postfix}-generated.rs");
